...
void generateNfaGraph(const NFA& nfa) {
    ...
    for (int i = 0; i < nfa.transition.size(); ++i) {
        if (nfa.isTerminal[i]) {
            outFile << "    " << i << " [shape=doublecircle];" << std::endl; // Для терминальных состояний
        } else {
            outFile << "    " << i << " [shape=circle];" << std::endl; // Для нетерминальных состояний
        }
    }
...
void generateDfaGraph(const DFA& dfa) {
    ...
    outFile << "    rankdir=LR;" << std::endl;  // Левый направленный граф

    // Пример нумерации состояний, создаем уникальные имена для состояний
    for (int i = 0; i < dfa.transition.size(); ++i) {
        if (dfa.isTerminal[i]) {
            outFile << "    " << i << " [shape=doublecircle];" << std::endl;
        } else {
            outFile << "    " << i << " [shape=circle];" << std::endl;
        }
    }
